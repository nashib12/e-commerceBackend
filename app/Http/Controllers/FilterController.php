...
class FilterController extends Controller
{
    public function categoryFilter(string $slug) {
        $category = Categories::where('slug', $slug)->with('allChildren')->firstOrFail();
        
        $categgoryIds = $category->getAllDescendantIds();
        
        $data = Products::select('id','name','base_price','sale_price','is_featured','slug','category_id','sale_price')->with(['primaryImage:id,product_id,url', 'categories:id,title,slug'])
                ->whereIn('category_id', $categgoryIds)->paginate(5);
    ...
    // public function productFilter(string $slug) {
